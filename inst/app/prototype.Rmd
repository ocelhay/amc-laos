...
hchart(dta, type = "column", hcaes(x = diasite, y = pct, color = color)) %>%
  hc_tooltip(headerFormat = "", pointFormat = "{point.diasite}: {point.pct}% <br>({point.n} of {point.total_presciptions} outpatient prescriptions)") %>%
  hc_title(text = i18n$t("Outpatient")) %>%
  hc_yAxis(title = list(text = i18n$t("% of Antimicrobial Prescriptions"))) %>%
  hc_xAxis(title = "") %>%
  hc_chart(marginLeft = 150, marginBottom = 150)
```
